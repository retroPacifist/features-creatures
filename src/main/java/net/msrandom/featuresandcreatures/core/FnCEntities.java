...
public class FnCEntities {
    ...
    public static final EntityType<Jockey> JOCKEY = createMobEntity("jockey", EntityType.Builder.of(Jockey::new, MobCategory.CREATURE).sized(0.35f, 1f), 0xDBA5FF, 0x564237);
    ...
    private static <T extends Mob> EntityType<T> createMobEntity(String name, EntityType.Builder<T> builder, int colour1, int colour2) {
        EntityType<T> type = builder.build(name);
        FnCItems.REGISTRAR.add(name + "_spawn_egg", new ForgeSpawnEggItem(() -> type, colour1, colour2, new Item.Properties().tab(FnCItems.TAB)));
